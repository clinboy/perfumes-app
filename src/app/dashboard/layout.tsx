...
function DashboardInner({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    ...(isSuperAdmin ? [{ href: "/dashboard/finanzas", label: "Finanzas", icon: "📈" }] : []),
    ...(isAdmin ? [{ href: "/dashboard/admin/usuarios", label: "Usuarios", icon: "👥" }] : []),
  ];

  const bottomNavItems: Array<{ href: string; label: string; icon: React.ReactNode; highlight?: boolean }> = [
    { href: "/dashboard", label: "Inicio", icon: (
  ...
  if (isAdmin) {
    bottomNavItems.push(
      { href: "/dashboard/productos/nuevo", label: "Nuevo", highlight: true, icon: (
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <span className="text-[11px] font-bold bg-white/20 px-2.5 py-1 rounded-full text-white/90">
              {isSuperAdmin ? "👑 Super Admin" : isAdmin ? "👑 Admin" : "🧑‍💼 " + userName}
            </span>
            <button
              onClick={toggle}
